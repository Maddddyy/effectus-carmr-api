...
async def run_pipeline(
    ...
    yield sse("stage_start", {"stage": "ingest", "stage_num": 0, "total_stages": 8,
                              "message": f"Parsing {len(files)} document(s)..."})
    ...
    yield sse("stage_complete", {"stage": "ingest",
                                  "message": f"Parsed {len(parsed_docs)} doc(s), {len(doc_context):,} chars"})

    # ── STAGE 1: Pre-Analysis ─────────────────────────────────────────────────
    yield sse("stage_start", {"stage": "preanalysis", "stage_num": 1, "total_stages": 8,
                              "message": "Mapping structural argument — claims, grounds, implicit premises..."})
    ...
    yield sse("thinking", {"stage": "preanalysis",
                           "message": f"Implicit grounds found: {len(preanalysis.get('implicit_grounds', []))}"})
    yield sse("thinking", {"stage": "preanalysis",
                           "message": f"Contested terms: {preanalysis.get('contested_terms', [])}"})
    ...
    yield sse("stage_start", {"stage": "commitment", "stage_num": 2, "total_stages": 8,
                              "message": "Extracting commitment — 4-part statement, ownership, reversibility..."})

    yield sse("thinking", {"stage": "commitment", "message": "Claude Opus analysing with extended thinking..."})
    ...
    yield sse("research", {"stage": "commitment", "query": "Validating strategic context + adversarial evidence",
                           "status": "searching..."})
    try:
        research_text = await research_commitment(
            title=c_result.get("title", ""),
            sector=sector,
            commitment_what=c_result.get("statementWhat", ""),
        )
        if research_text and "[No research" not in research_text:
            research_citations.append(f"Commitment context: {c_result.get('title', '')}")
            yield sse("research", {"stage": "commitment", "query": "Strategic context + counter-evidence",
                                   "finding": research_text[:400], "status": "complete"})
            ...
            yield sse("thinking", {"stage": "commitment",
                                   "message": f"Revised with research. Quality: {c_score:.2f}"})
    except Exception as e:
        logger.warning(f"Commitment research failed: {e}")
        yield sse("thinking", {"stage": "commitment", "message": "Research unavailable — proceeding"})
    ...
    yield sse("stage_start", {"stage": "assumptions", "stage_num": 3, "total_stages": 8,
                              "message": "Argumentation analysis — extracting defeasible premises (max 5, parent-child)..."})

    yield sse("thinking", {"stage": "assumptions",
                           "message": "Pass 1: Drafting assumptions using 4 argumentation tests..."})
    a_result, a_score, a_issues = await run_assumptions(running_context)
    assumptions_list = a_result.get("assumptions", [])

    yield sse("thinking", {"stage": "assumptions",
                           "message": f"QC pass complete: {len(assumptions_list)} assumptions, score {a_score:.2f}"})
        ...
        try:
            ...
            if finding and "[No research" not in finding:
                ...
                if any(s in finding.lower() for s in at_risk_signals):
                    a["status"] = "at-risk"
                    yield sse("thinking", {"stage": "assumptions",
                                           "message": f"Research signals {a.get('id')} may already be at-risk"})
        except Exception as e:
    ...
    yield sse("stage_start", {"stage": "reasoning", "stage_num": 4, "total_stages": 8,
                              "message": "Walton's Practical Reasoning — causal strands with defeat analysis (max 3)..."})

    yield sse("thinking", {"stage": "reasoning", "message": "Extracting argument schemes with warrants and undercutters..."})
    ...
    yield sse("stage_start", {"stage": "meaning", "stage_num": 5, "total_stages": 8,
                              "message": "Equivocation prevention — contested terms in governance-critical clauses only..."})

    yield sse("thinking", {"stage": "meaning",
                           "message": "Identifying terms whose drift would break falsification conditions..."})
    ...
    for mt in meaning_terms[:2]:  # Max 2 research calls
        term = mt.get("term", "")
        yield sse("research", {"stage": "meaning", "query": f"'{term}' industry definition",
                               "status": "searching..."})
        try:
            sector = carmr.recordData.portfolio or carmr.recordData.title[:40]
            finding = await research_term_definition(term, sector)
            if finding and "[No research" not in finding:
                research_citations.append(f"Term definition: '{term}'")
                yield sse("research", {"stage": "meaning", "query": f"'{term}' definition",
                                       "finding": finding[:350], "status": "complete"})
        except Exception as e:
    ...
    yield sse("stage_start", {"stage": "review", "stage_num": 6, "total_stages": 8,
                              "message": "Generating falsification-style review triggers..."})
    ...
    yield sse("stage_start", {"stage": "cross_validation", "stage_num": 7, "total_stages": 8,
                              "message": "Structural integrity check — assumption coverage, falsification completeness..."})
    ...
    yield sse("stage_start", {"stage": "synthesis", "stage_num": 8, "total_stages": 8,
                              "message": "Assembling final CARMR record..."})
    ...
    result = ExtractionResult(
        ...
        research_citations=list(set(research_citations)),
